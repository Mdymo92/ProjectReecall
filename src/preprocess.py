...
@app.command()
def preprocess(
    input_dir: str = typer.Option(..., help="Directory of intermediate JSONC files"),
    output_dir: str = typer.Option(..., help="Directory for cleaned output"),
    ...
    for idx in range(start_idx, total, batch_size):
        ...
        logger.info(f"Processing batch {idx} to {idx + len(batch)-1} of {total-1}")
        for file in batch:
            try:
                ...
                logger.error(f"Error on {file.name}: {e}")
